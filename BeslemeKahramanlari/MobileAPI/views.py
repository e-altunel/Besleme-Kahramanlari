...
def get_user_posts(request):
    if not request.user or not request.user.is_active:
        return Response({"error": "User Not Found"}, status=HTTP_404_NOT_FOUND)
    posts_serial = PostSerializer(
        instance=Post.objects.order_by("-created_at")
...
def get_feed_points(request):
    ...
    if latitude is None or longitude is None:
        return Response(
            {"error": "Latitude or Longitude is missing"}, status=HTTP_400_BAD_REQUEST
        )

    feed_points = FeedPoint.objects.all()
    feed_points.filter(
        latitude__gte=latitude - 0.1, latitude__lte=latitude + 0.1
    ).filter(longitude__gte=longitude - 0.1, longitude__lte=longitude + 0.1)
...
def share_post(request):
    ...
    if serializer.is_valid():
        ...
        post.user.save()
        post.feed_point.should_update = True
...
def change_password(request):
    ...
    if old_password is None or new_password is None:
        return Response(
            {"error": "Old or New Password is missing"}, status=HTTP_400_BAD_REQUEST
        )
    if not request.user.check_password(old_password):
        return Response({"error": "Old Password is wrong"}, status=HTTP_400_BAD_REQUEST)
    if old_password == new_password:
        return Response(
            {"error": "Old and New Passwords are same"}, status=HTTP_400_BAD_REQUEST
        )
    if BeslemeKahramani.is_password_valid(new_password):
        return Response(
            {"error": "New Password is not valid"}, status=HTTP_400_BAD_REQUEST
        )
